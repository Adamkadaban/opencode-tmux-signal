...
- **Smart window naming** — names the window from a short LLM slug of your first prompt (`fix the auth token bug` → `auth`), and **never overwrites a name you set** (it only renames a window still called `opencode`)
...
| `OPENCODE_TMUX_SIGNAL_WINDOW_NAME` | `llm` | `llm` = short LLM slug from first prompt; `dir` = project dir; `off` = leave it |
